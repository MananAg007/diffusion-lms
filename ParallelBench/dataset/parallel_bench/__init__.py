...
class ParallelBench:
    ...
    def compute_metrics(self, predictions, references, output_per_sample=False, **kwargs):
        ...
        # Check if metric function supports batch processing
        if hasattr(self.metric_func, 'compute_batch'):
            # Use batch processing (much faster for BERTScore!)
            metrics_per_sample = self.metric_func.compute_batch(predictions, references)
        else:
