...
class ParaphraseScore(Metric):
    ...
    def compute_batch(self, predictions, ground_truths, strict=False):
        """Compute metrics for a batch of predictions (more efficient for BERTScore)."""
        self.load()

        predictions = [p.strip() for p in predictions]
        texts = [gt["text"] for gt in ground_truths]

        # Compute BERTScore in batch (much faster!)
        non_empty_mask = [p != "" for p in predictions]
        non_empty_predictions = [p for p, mask in zip(predictions, non_empty_mask) if mask]
        non_empty_texts = [t for t, mask in zip(texts, non_empty_mask) if mask]

        # Initialize result arrays
        inv_bleu_scores = []
        bertscore_scores = [0.0] * len(predictions)
        grammar_scores = []

        # Compute BLEU per sample (it's already fast)
        for pred, text in zip(predictions, texts):
            if pred != "":
                inv_bleu = 1 - self.bleu.compute(predictions=[pred], references=[[text]])["bleu"]
                inv_bleu_scores.append(inv_bleu)
                grammar_scores.append(float(grammar_check(pred)))
            else:
                inv_bleu_scores.append(0.0)
                grammar_scores.append(0.0)

        # Compute BERTScore in batch for non-empty predictions
        if non_empty_predictions:
            import torch
            device = "cuda" if torch.cuda.is_available() else "cpu"
            bert_results = self.bertscore.compute(
                predictions=non_empty_predictions,
                references=non_empty_texts,
                lang="en",
                device=device  # Use GPU if available!
            )
            # Fill in BERTScore results for non-empty predictions
            bert_idx = 0
            for i, mask in enumerate(non_empty_mask):
                if mask:
                    bertscore_scores[i] = float(bert_results["f1"][bert_idx])
                    bert_idx += 1

        # Combine metrics per sample
        metrics_per_sample = []
        for inv_bleu, bertscore, grammar in zip(inv_bleu_scores, bertscore_scores, grammar_scores):
            metrics_per_sample.append({
                "inv_bleu_score": float(inv_bleu),
                "bertscore_score": float(bertscore),
                "grammar_score": float(grammar),
                "score": float(inv_bleu * bertscore * grammar),
            })
