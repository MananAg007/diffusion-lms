...
class LladaGenerationConfig:
    ...
    pool_size: int = 1  # M - number of top tokens to consider per position (1 = normal generate, >1 = compatibility checking)
    score_function: str = "perplexity"  # Scoring method for compatibility checking: 'grammar', 'perplexity', or 'ngram'
    ngram_size: int = 3  # N-gram size for ngram scoring (default: 3 for trigrams)
...
class LladaModel(BaseModel):
    ...
    def model_generate(self, input_ids, gen_config, output_history=False, output0_ids=None, question_id=None):
        ...
        if gen_config.is_mdpo_rcr():
            ...
            assert not use_compatibility, "Compatibility checking is not supported with MDPO-RCR"
            ...
            assert not use_compatibility, "Compatibility checking is not supported with ReMDM"
        ...
        elif use_compatibility:
            # Use compatibility checking mode
            assert self.accel_framework is None, "Compatibility checking is not supported with fast-dllm acceleration"
            generate_fn = functools.partial(generate_with_compatibility, tokenizer=self.tokenizer)
