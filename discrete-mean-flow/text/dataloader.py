...
def get_dataset(dataset_name,
    ...
    if dataset_name == 'wikitext103':
        ...
        dataset = datasets.load_dataset(
            ...
            num_proc=num_proc,
            trust_remote_code=True)
    elif dataset_name == 'Skylion007/openwebtext-1k':
        ...
        dataset = datasets.load_dataset(
            ...
            streaming=False,
            trust_remote_code=True)
    elif dataset_name == 'openwebtext-20':
        dataset = datasets.load_dataset(
            'Skylion007/openwebtext',
            split='train[:20]',
            cache_dir=cache_dir,
            ...
            streaming=False,
            trust_remote_code=True)
    elif dataset_name == 'openwebtext-1sample':
        # Single sample for extreme overfitting test
        # Loads pre-tokenized data from npy file (saved from notebook)
        x0 = np.load(
            "/dataset/david/discrete-mean-flow/text/owt_1sample_data/x0.npy")

        # Dynamically cut to match model sequence length (block_size)
        # x0 shape is expected to be [1, original_seq_len]
        original_seq_len = x0.shape[1] if len(x0.shape) > 1 else x0.shape[0]

        if original_seq_len > block_size:
            # Cut to block_size
            if len(x0.shape) > 1:
                x0 = x0[:, :block_size]  # [1, block_size]
            else:
                x0 = x0[:block_size].reshape(1, -1)  # [1, block_size]
        elif original_seq_len < block_size:
            # Pad to block_size if needed (though unlikely for overfitting test)
            if len(x0.shape) > 1:
                pad_length = block_size - original_seq_len
                padding = np.zeros((x0.shape[0], pad_length), dtype=x0.dtype)
                x0 = np.concatenate([x0, padding], axis=1)
            else:
                x0 = x0.reshape(1, -1)
                pad_length = block_size - x0.shape[1]
                padding = np.zeros((1, pad_length), dtype=x0.dtype)
                x0 = np.concatenate([x0, padding], axis=1)

        # Create dataset with proper format
        single_sample_ds = datasets.Dataset.from_dict({
            'input_ids': torch.from_numpy(x0),
            'attention_mask': torch.ones_like(torch.from_numpy(x0)),
        })
        single_sample_ds.set_format(type='torch')

        # Return the single sample dataset directly (already tokenized)
        # Same data for both train and validation
        return single_sample_ds
    elif dataset_name == 'text8-1sample':
        # Single text8 sample for extreme overfitting test
        # Loads pre-tokenized data from npy file (saved from notebook)
        x0 = np.load(
            "/data2/david3684/discrete-mean-flow/text/text8_1sample_data/x0.npy")
