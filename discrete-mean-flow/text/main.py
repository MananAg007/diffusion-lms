...
def _generate_samples(diffusion_model, config, logger,
    ...
    if config.mode == 'sample_eval_recon':
        sample_sample = np.load("/dataset/david/discrete-mean-flow/text/lm1b_1sample_data/x0.npy")
    else:
        sample_sample = None
    for _ in range(config.sampling.num_sample_batches):
        if config.sampling.semi_ar:
            ...
            samples = model.restore_model_and_sample(
                num_steps=config.sampling.steps, sample_sample=sample_sample)
...
def main(config):
    ...
    elif config.algo.name == 'dos':
        diffusion_model = algo.DOS
    elif config.algo.name == 'dos_distill':
        diffusion_model = algo.DOS_distill
